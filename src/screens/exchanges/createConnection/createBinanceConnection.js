...
export default class CreateBinanceConnection extends Component {
    constructor(props) {
        super(props);
        const { item, apiKey, apiSecret } = props.navigation.state.params;
        this.state = {
            portfolio: "Main Portfolio",
            name: item.name + ' Connect',
            apiKey: 'fK6p9SrbLmn6GdGRmw8DY5SU6z8cJplyc4S2FWVNzeMrHPp1nlL41p82qSSUkcpx',
            apiSecret: 'OgYXX37L7PN2rQgl4d6omgbW4Rqx2YsR6HtyG8D1BOflG1x1MPg7F2cI66QMvZVy',
    ...
    loginBinance = (data) => {
        ...
        POST(EXCHANGES.LOGIN_BINANCE, config)
            .then((res) => {
                console.log('binance login res', res);
                this.props.navigation.navigate('wallets', {name: 'binance'});
            })
            .catch((error) => {
                console.log(error)
    ...
    render() {
        const { portfolio, name, apiKey, apiSecret, additionalOptions, loading } = this.state;
