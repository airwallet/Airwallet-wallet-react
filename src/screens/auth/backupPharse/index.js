...
import { View, Text, StyleSheet, TouchableOpacity, Platform, ScrollView, BackHandler, ToastAndroid } from 'react-native';
...
import VerifyPharse from './verifyPharse';
import Pharse from './pharse';
import { setAsyncStorage, getAsyncStorage, setMultiAsyncStorage } from '../../../utils/asyncStorage';
import { LOGIN_STATUS, USER_DATA, ACCESS_TOKEN, ENDPOINTS, BACKUP_PHRASE, USER_ID } from '../../../constants/api';
...
class BackupPharse extends Component {
    ...
    componentWillMount(){
        // setMultiAsyncStorage(
        //     [[BACKUP_PHRASE, JSON.stringify(true)]])
        getAsyncStorage(USER_DATA).then(data => {
			...
			if(userData){
				...
				POST(ENDPOINTS.GENERATE_BACKUP_PHRASE, config)
				.then(res => {
                    console.log('backupphrase', res)
                    const { backupPhrase } = res.data
                    this.setState({generatingPhrase: false, backupPhrase})
				})
				.catch(error => {
					this.setState({generatingPhrase: false})
                    console.log(error)
      ...
      componentDidMount() {
        ...
        onBackButtonPressAndroid = () => {
            const {verify, match} = this.state;
            if(verify){
                this.setState({verify: false, match: false})
                return true;
            }else{
                return false
            }
    ...
    onContinue = () => {
        // this.setState({verify: !this.state.verify, match: false});
        const { verify, match, errorShake } = this.state;
        if(!match && verify){
            this.setState({errorShake: !errorShake});
            return
        }
        if(!verify){
            this.setState({verify: true, match: false})
        }else if(verify){
            const { userData, selectedWords } = this.state;
            this.setState({verifing: true})
            if(userData[ACCESS_TOKEN]){
				const config = {
                    headers: {Authorization: 'bearer ' + userData[ACCESS_TOKEN]},
                    data: {backupPhrase: selectedWords}
				}
				...
				.then(res => {
                    ...
                    setMultiAsyncStorage(
                        ...
                        [BACKUP_PHRASE, JSON.stringify(selectedWords)]]
                    )
    ...
    render() {
        const { backupPhrase, verify, generatingPhrase, match, verifing, errorShake } = this.state;
        let secretString = '';
            ...
            <Background type="dark" style={styles.background}>
                ...
                <ScrollView>
                    <View style={styles.header}>
                        ...
                        <Text style={styles.headerTxt}>{`${verify ? Languages.VERIFY : Languages.BACKUP} ${Languages.PHRASE}`}</Text>
                    </View>

                        {!generatingPhrase ? (verify ? 
                            <VerifyPharse 
                                backupPhrase={backupPhrase}
                                errorShake={errorShake}
                                matchWords={(match, selectedWords) => this.setState({match, selectedWords})}
                            /> 
                            : 
                            <Pharse generatingPhrase={generatingPhrase} secretString={secretString}/>) 
                            : <Loader loadingTxt="Generating phrase"/>}
                   </ScrollView>
                </Container>
                <BottomButton loading={verifing}>
                    <TouchableOpacity disabled={generatingPhrase} onPress={this.onContinue} activeOpacity={0.6} style={styles.bottomButton}>
